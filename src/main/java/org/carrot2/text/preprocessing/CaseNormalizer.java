...
/**
 * Performs case normalization and calculates a number of frequency statistics
 * for words. The aim of case normalization is to find the most frequently
 * appearing variants of words in terms of case. For example, if in the input
 * documents <i>MacOS</i> appears 20 times, <i>Macos</i> 5 times and
 * <i>macos</i> 2 times, case normalizer will select <i>MacOS</i> to represent
 * all variants and assign the aggregated term frequency of 27 to it.
 * <p>
...
//		boolean isChinese = context.language.getLanguageCode().equals(LanguageCode.CHINESE_SIMPLIFIED);
...
//			final boolean sameImage = isChinese ? sameCase
//					: CharArrayComparators.CASE_INSENSITIVE_CHAR_ARRAY_COMPARATOR.compare(image, nextImage) == 0;

			final boolean sameImage = CharArrayComparators.CASE_INSENSITIVE_CHAR_ARRAY_COMPARATOR.compare(image, nextImage) == 0;
	...
	private boolean isNotIndexed(final int tokenType) {
		return tokenType == ITokenizer.TT_PUNCTUATION || tokenType == ITokenizer.TT_FULL_URL
