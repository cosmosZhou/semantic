...
import com.patsnap.core.analysis.manager.StopWordManager;
 ...
 * </ul>
 * <p>
 * This class requires that {@link Tokenizer} and {@link CaseNormalizer} be
 * invoked first.
 */
	...
	public void mark(PreprocessingContext context) {
		...
		for (int i = 0; i < wordImages.length; i++) {
			...
			case CHINESE_SIMPLIFIED:				
				isStopWord = StopWordManager.Chinese.instance.isCommonWord(mutableCharArray.toString());
//				isStopWord = lexData.isCommonWord(mutableCharArray);
				break;
			case ENGLISH:
				isStopWord = StopWordManager.English.instance.isCommonWord(mutableCharArray.toString());
//				isStopWord = lexData.isCommonWord(mutableCharArray);
				break;
			default:
				isStopWord = lexData.isCommonWord(mutableCharArray);
