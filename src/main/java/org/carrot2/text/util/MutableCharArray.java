 ...
 * {@link #equals(Object)} against other {@link MutableCharArray} objects,
 * assuming the underlying character buffers does not change. In case the
 * buffers is changed, the resulting behavior is unpredictable.
 */
public final class MutableCharArray implements CharSequence, Cloneable {
	private static final char[] EMPTY = new char[0];

	/**
	 * Internal buffer with character data. The buffer may be smaller or larger than
	 * the actual sequence represented by this object.
	 */
	private char[] buffer;
	...
	/**
	 * Creates a {@link MutableCharArray} from another {@link CharSequence}, creates
	 * a new buffer to store characters.
	 */
	...
	/**
	 * Resets internal buffers in this object to represent another character
	 * sequence. See class header notes for side-effects on {@link #equals(Object)}
	 * and {@link #hashCode()}.
	 */
	 ...
	 * See class header notes for side-effects on {@link #equals(Object)} and
	 * {@link #hashCode()}.
	 */
	...
	public void simplify_fullwidth() {
		for (int i = 0; i < length; ++i) {
			if (buffer[i] >= 0xFF10)
				buffer[i] -= 0xFEE0;
		}
	...
	/**
	 * @return Returns the internal buffer <i>currently</i> used to store the
	 *         content of this char sequence.
	 */
	...
	/**
	 * Calculates a hash code for a given portion of the character buffer. The
	 * returned value is identical to what be returned from {@link #hashCode()} if a
	 * wrapper {@link MutableCharArray} were created.
	 */
