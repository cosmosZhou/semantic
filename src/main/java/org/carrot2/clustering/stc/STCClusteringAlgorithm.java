...
/**
 * Suffix Tree Clustering (STC) algorithm. Pretty much as described in: <i>Oren
 * Zamir, Oren Etzioni, Grouper: A Dynamic Clustering Interface to Web Search
 * Results, 1999.</i> Some liberties were taken wherever STC's description was
 * not clear enough or where we thought some improvements could be made.
 */
...
public final class STCClusteringAlgorithm extends ProcessingComponentBase implements IClusteringAlgorithm {
	...
	/**
	 * Query that produced the documents. The query will help the algorithm to
	 * create better clusters. Therefore, providing the query is optional but
	 * desirable.
	 */
	@Processing
	@Input
	...
	/**
	 * Maximum word-document ratio. A number between 0 and 1, if a word exists in
	 * more snippets than this ratio, it is ignored.
	 */
	@Processing
	@Input
	@Attribute
	...
	/**
	 * Maximum base clusters count. Trims the base cluster array after N-th position
...
//	public int maxBaseClusters = 300;
	public int maxBaseClusters = 3000;
...
//	public int maxClusters = 15;
	...
	/**
	 * Maximum words per label. Base clusters formed by phrases with more words than
	 * this ratio are trimmed.
	 */
	@Processing
	@Input
	@Attribute
	...
	/**
	 * Maximum phrases per label. Maximum number of phrases from base clusters
	 * promoted to the cluster's label.
	 */
	@Processing
	@Input
	@Attribute
	...
	/**
	 * Single term boost. A factor in calculation of the base cluster score. If
	 * greater then zero, single-term base clusters are assigned this value
	...
	/**
	 * Document count boost. A factor in calculation of the base cluster score,
	 * boosting the score depending on the number of documents found in the base
	 * cluster.
	 */
	@Processing
	@Input
	...
	/**
	 * Balance between cluster score and size during cluster sorting. Value equal to
	 * 0.0 will sort clusters based only on cluster size. Value equal to 1.0 will
	 * sort clusters based only on cluster score.
	 */
	@Input
	@Processing
	@Attribute
	...
	public void process() throws ProcessingException {
		...
		clusters = multilingualClustering.process(documents, new IMonolingualClusteringAlgorithm() {
	...
	/**
	 * Performs the actual clustering with an assumption that all documents are
	 * written in one <code>language</code>.
	 */
	private void cluster(LanguageCode language) {
		clusters = new ArrayList<Cluster>();

		/*
		 * Step 1. Preprocessing: tokenization, stop word marking and stemming (if
		 * available).
		 */
		Utility.Timer timer = new Utility.Timer();
		context = preprocessingPipeline.preprocess(documents, query, language);
		timer.report("preprocessingPipeline.preprocess(documents, query, language)");
...
//					phrase += new String(context.allWords.image[tokenIndex[s + j]]);
//				}
//				System.out.println("key phrase: " + String.join("", phrase));
//
//				boolean isKeyPhrase;
//				switch (context.language.getLanguageCode()) {
//				case CHINESE_SIMPLIFIED:
//					isKeyPhrase = phrase.length() <= 8 && Native.keywordCN(phrase) > 0.5;
//					break;
//				case ENGLISH:
//					isKeyPhrase = phrase.length() <= 20 && Native.keywordEN(phrase) > 0.5;
//					break;
//				default:
//					isKeyPhrase = true;
//				}
...
//					sb.addPhrase(tokenIndex, s, phraseLength);
		...
		/*
		 * Step 3: Find "base" clusters by looking up frequently recurring phrases in
		...
		timer.report("STCClusteringAlgorithm");
	}
	 ...
	 * phrases. We extract them by walking the generalized suffix tree constructed
	 * for each phrase, and extracting paths from those internal tree states, that
	 * occurred in more than one document.
	 */
	private List<ClusterCandidate> createBaseClusters(SequenceBuilder sb) {
		/*
		 * Collect all phrases that will form base clusters, initially filtered to
		 * fulfill the minimum acceptance criteria.
		 */
		...
		/*
		 * We limit the number of base clusters to the one requested by the user. First
		 * we sort by the base clusters score, then pick the top-K entries, filtering
		 * out any stop labels on the way.
		 */
	...
	/**
	 * Create final clusters by merging base clusters and pruning their labels.
	 * Cluster merging is a greedy process of compacting clusters with document sets
	 * that overlap by a certain ratio. In other words, phrases that "cover" nearly
	 * identical document sets will be conflated.
	 */
	private ArrayList<ClusterCandidate> createMergedClusters(List<ClusterCandidate> baseClusters) {
		...
		final ArrayList<ClusterCandidate> mergedClusters = Lists.newArrayListWithCapacity(baseClusters.size());
		...
		for (int v = 0; v < baseClusters.size(); v++) {
			...
			/*
			 * Aggregate documents from each base cluster of the current merge, compute the
			 * score and labels.
			 */
	...
	/**
	 * Leave only most general (no other phrase is a substring of this one) and most
	 * specific (no other phrase is a superstring of this one) phrases.
	 */
	private void markSubSuperPhrases(ArrayList<PhraseCandidate> phrases) {
		...
		for (int i = 0; i < max; i++) {
			for (int j = 0; j < max; j++) {
				...
				int index = indexOf(words.buffer, offsets.get(2 * i), offsets.get(2 * i + 1), words.buffer,
						offsets.get(2 * j), offsets.get(2 * j + 1));
				if (index >= 0) {
		...
		for (int i = 0; i < max; i++) {
			...
			for (int j = 0; j < max; j++) {
				...
				if (i == j || !b.mostSpecific)
					continue;

				int index = indexOf(words.buffer, offsets.get(2 * j), offsets.get(2 * j + 1), words.buffer,
						offsets.get(2 * i), offsets.get(2 * i + 1));
				if (index >= 0) {
	...
	private void markOverlappingPhrases(ArrayList<PhraseCandidate> phrases) {
		...
		for (int i = 0; i < max; i++) {
			for (int j = i + 1; j < max; j++) {
				...
				final float intersection = computeIntersection(words.buffer, offsets.get(2 * i), a_words, words.buffer,
						offsets.get(2 * j), b_words);
	...
	/**
	 * Create the junk (unassigned documents) cluster and create the final set of
	 * clusters in Carrot2 format.
	 */
	...
	private String buildLabel(int[] phraseIndices) {
		...
		return LabelFormatter.format(images, stopwords, context.language.getLanguageCode().usesSpaceDelimiters());
	}

	@SuppressWarnings("unused")
	private String toString(PhraseCandidate c) {
		return String.format(Locale.ENGLISH, "%3.2f %s %s %s %s", c.coverage, buildLabel(c.cluster.phrases.get(0)),
				c.selected ? "S" : "", c.mostGeneral ? "MG" : "", c.mostSpecific ? "MS" : "");
	}

	/**
	 * Build a cluster's label from suffix tree edge indices, including some
	...
	/**
	 * Consider certain special cases of internal suffix tree nodes. The suffix tree
	 * may contain internal nodes with paths starting or ending with a stop word
	 * (common word). We have the following interesting scenarios:
	 * 
	 * <dl>
	 * <dt>IF LEADING STOPWORD: IGNORE THE NODE.</dt>
	 * <dd>There MUST be a phrase with this stopword chopped off in the suffix tree
	 * (a suffix of this phrase) and its frequency will be just as high.</dd>
	 * 
	 * <dt>IF TRAILING STOPWORDS:</dt>
	 * <dd>Check if the edge leading to the current node is composed entirely of
	 * stopwords. If so, there must be a parent node that contains non-stopwords and
	 * we can ignore the current node. Otherwise we can chop off the trailing
	 * stopwords from the current node's phrase (this phrase cannot be duplicated
	 * anywhere in the tree because if it were, there would have to be a branch
	 ...
	 * <p>
	 * The boost is calculated as a Gaussian function of density around the
	 * "optimum" expected phrase length (average) and "tolerance" towards shorter
	 * and longer phrases (standard deviation). You can draw this score multiplier's
	 * characteristic with gnuplot:
	 * 
	 * <pre>
	 * reset
	 * 
	 ...
	 * </pre>
	 * 
	 * One word-phrases can be given a fixed boost, if {@link #singleTermBoost} is
	 * greater than zero.
	 * 
	 ...
	 *         of documents the phrase occurred in and a function of the effective
	...
	final float baseClusterScore(final int phraseLength, final int documentCount) {
		...
		if (phraseLength == 1 && singleTermBoost > 0) {
			...
			boost = Math.exp((-tmp * tmp) / (2 * optimalPhraseLengthDev * optimalPhraseLengthDev));
		}
	 ...
	 */
	private static int indexOf(int[] source, int sourceOffset, int sourceCount, int[] target, int targetOffset,
			int targetCount) {
		if (targetCount == 0) {
			return 0;
		}
