...
/**
 * Lingo clustering algorithm. Implementation as described in: <i> "Stanisław
 * Osiński, Dawid Weiss: A Concept-Driven Algorithm for Clustering Search
 * Results. IEEE Intelligent Systems, May/June, 3 (vol. 20), 2005, pp.
 * 48—54."</i>.
 */
...
public class LingoClusteringAlgorithm extends ProcessingComponentBase implements IClusteringAlgorithm {
	 ...
	 * 0.0 will cause Lingo to sort clusters based only on cluster size. Value equal
	 * to 1.0 will cause Lingo to sort clusters based only on cluster score.
	 */
	@Input
	@Processing
	@Attribute
	...
	/**
	 * Desired cluster count base. Base factor used to calculate the number of
	 * clusters based on the number of documents on input. The larger the value, the
	 * more clusters will be created. The number of clusters created by the
	 * algorithm will be proportional to the cluster count base, but not in a linear
	 * way.
	 */
	@Input
	@Processing
	...
	private void cluster(LanguageCode language) {
		...
		final PreprocessingContext context = preprocessingPipeline.preprocess(documents, query, language);
		...
		if (context.hasLabels()) {
			...
			matrixReducer.reduce(reducedVsmContext, computeClusterCount(desiredClusterCountBase, documents.size()));
		...
		timer.report("LingoClusteringAlgorithm");
	}

	/**
	 * Computes the number of clusters to create based on a very simple heuristic
	 * based on the number of documents on input.
	 */
