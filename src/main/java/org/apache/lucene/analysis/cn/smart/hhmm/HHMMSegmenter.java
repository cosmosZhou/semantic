...
public class HHMMSegmenter {
	...
	private SegGraph createSegGraph(String sentence) {
		...
		while (i < length) {
			...
			case CharType.HANZI:
				...
				// It doesn't matter if a single Chinese character (Hanzi) can form a phrase or
				// not,
				// it will store that single Chinese character (Hanzi) in the SegGraph.
				// Otherwise, it will
				// cause word division.
	 ...
	 * @return array of character types corresponding to character positions in the
	 *         sentence
	 */
	...
	public List<SegToken> process(String sentence) {
		List<SegToken> list;
//		if (WordSegmenter.useCRF) {
//			String seg[] = Native.segmentCN(sentence);
//			list = new ArrayList<SegToken>();
//
//			list.add(new SegToken(Utility.START_CHAR_ARRAY, -1, 0, WordType.SENTENCE_BEGIN, 0));
//
//			int start = 0;
//			for (String lexeme : seg) {
//				int end = start + lexeme.length();
//				list.add(new SegToken(lexeme.toCharArray(), start, end, Utility.getWordType(lexeme), 0));
//				start = end;
//			}
//
//			list.add(new SegToken(Utility.END_CHAR_ARRAY, sentence.length(), sentence.length() + 1,
//					WordType.SENTENCE_END, 0));
//			return list;
//		} else {
		SegGraph segGraph = createSegGraph(sentence);
		BiSegGraph biSegGraph = new BiSegGraph(segGraph);
		list = biSegGraph.getShortPath();
//		}
