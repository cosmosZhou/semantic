...
public class Utility {
	...
	public static int getWordType(MutableCharArray word) {
		switch (getCharType(word.charAt(0))) {
		case CharType.DELIMITER:
			return WordType.DELIMITER;

		case CharType.LETTER:
			for (int i = 1; i < word.length(); i++) {
				if (getCharType(word.charAt(i)) == CharType.FULLWIDTH_LETTER) {
					return WordType.FULLWIDTH_STRING;
				}
			}
		...
		case CharType.DIGIT:
			for (int i = 1; i < word.length(); i++) {
				if (getCharType(word.charAt(i)) == CharType.FULLWIDTH_DIGIT) {
					return WordType.FULLWIDTH_NUMBER;
				}
			}
