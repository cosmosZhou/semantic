...
public final class Pattern implements java.io.Serializable {
	...
	public static class Node extends Object {
		...
		public String toStringSingleTrimmed() {
			return toStringSingle();
		}

		public String toStringTrimmed() {
			String str = toStringSingleTrimmed();
			if (next != null)
				str += next.toStringTrimmed();
			return str;
		}
	}
	...
	private static abstract class CharProperty extends Node {
		...
		CharProperty try_complement() {
			try {
				Field complementField = this.getClass().getDeclaredField("this$1");
				complementField.setAccessible(true);
				return (CharProperty) complementField.get(this);
			} catch (NoSuchFieldException | SecurityException | IllegalArgumentException | IllegalAccessException e) {
				return null;
			}
		}

		CharProperty[] try_binary(String op) {
			try {
				Field binaryField = this.getClass().getDeclaredField("val$" + op);
				binaryField.setAccessible(true);

				Field rhsField = this.getClass().getDeclaredField("val$rhs");
				rhsField.setAccessible(true);

				return new CharProperty[] { (CharProperty) binaryField.get(this), (CharProperty) rhsField.get(this) };
			} catch (NoSuchFieldException | SecurityException | IllegalArgumentException | IllegalAccessException e) {
				return null;
			}
		}
		...
		@Override
		public String toStringSingle() {
			CharProperty complement = try_complement();
			if (complement != null) {
				String negative = strip_bracket(complement.toStringSingle());
				switch (negative) {
				case "\\w":
					return "\\W";
				case "\\d":
				...
				case "\\D":
					return "\\d";
				default:
					return String.format("[^%s]", negative);
				}
			}
	...
	static class SliceNode extends Node {
		...
		@Override
		public String toStringSingleTrimmed() {
			return toStringSingle().replaceAll("\\s", "");
		}
	}
	...
	static final class Curly extends Node {
		...
		String format() {
			if (this.cmax != MAX_REPS || this.cmin > 1) {
				String regex = String.format("%%s{%d, %d}", this.cmin, this.cmax);
				switch (this.type) {
				case LAZY:
					return regex + '?';
				case POSSESSIVE:
		...
		@Override
		public String toStringSingle() {
			return String.format(format(), atom.toStringSingle());
		}

		@Override
		public String toStringSingleTrimmed() {
			String atom_str = atom.toStringSingle();
			switch (atom_str) {
			case " ":
			case "//s":
				return "";
			default:
				return String.format(format(), atom_str);
			}
		}
	}
	 ...
	 */
	private static CharProperty union(final CharProperty union, final CharProperty rhs) {
		return new CharProperty() {
			boolean isSatisfiedBy(int ch) {
				return union.isSatisfiedBy(ch) || rhs.isSatisfiedBy(ch);
			}
		};
	}
	 ...
	 */
	private static CharProperty intersection(final CharProperty intersection, final CharProperty rhs) {
		return new CharProperty() {
			boolean isSatisfiedBy(int ch) {
				return intersection.isSatisfiedBy(ch) && rhs.isSatisfiedBy(ch);
			}
		};
	}
	 ...
	 */
	private static CharProperty setDifference(final CharProperty setDifference, final CharProperty rhs) {
		return new CharProperty() {
			boolean isSatisfiedBy(int ch) {
				return !rhs.isSatisfiedBy(ch) && setDifference.isSatisfiedBy(ch);
			}
		};
	}
